...
## You are one of several agents on a shared Mac

Two Claude Code accounts (`claude`, `cx`), OpenCode, dsh, kimi,
grok and you run on the same repos, often at the same time. You share their claims, queue and guardrail files, so the coordination rules below are yours
...
| Memory | Relevant memories from the Claude accounts' shared memory (one store per folder since 2026-09-24) are recalled into your context per prompt (`hooks/memory_recall.py`). Durable learnings go in the project's `CLAUDE.md` or the vault, never only in session memory |
...
- **Session registry.** Codex does not write one, so `hooks/agent_hook.py --agent codex` writes
  `~/.codex/agent-sessions/<pid>-<id>.json` for you (`codex_hook.py` is a wrapper kept for the approved hooks.json). That is what makes other sessions see your claims.
...
- **Model policy still applies to you.** You are the only outside model allowed on confidential
  ...
  DeepSeek, Kimi, OpenCode, Gemini or Grok.
- **Auto-memory.** Yours is separate from the Claude accounts' shared memory and invisible to them. Anything
